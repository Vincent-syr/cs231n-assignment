...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        lay1_act, lay1_chche = affine_relu_forward(X, w1, b1)
        lay2_score, lay2_cache = affine_forward(lay1_act, w2, b2)
        scores = lay2_score
        ...
        dlay1_act, dw2, db2 = affine_backward(dout, lay2_cache)
        dx, dw1, db1 = affine_relu_backward(dlay1_act, lay1_chche)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(len(hidden_dims)):
            unit_num = hidden_dims[i]
            self.params['W'+str(i+1)] = np.random.normal(0, weight_scale, size=(input_dim, unit_num))
            self.params['b'+str(i+1)] = np.zeros(unit_num)
            if self.use_batchnorm:
                self.params['gamma'+str(i+1)] = np.ones(unit_num)
                self.params['beta'+str(i+1)] = np.zeros(unit_num)
            input_dim = unit_num
        self.params['W'+str(self.num_layers)] = np.random.normal(0, weight_scale, size=(unit_num, num_classes))
