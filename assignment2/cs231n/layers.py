...
def affine_forward(x, w, b):
    ...
    x_rev = x.reshape(x.shape[0], -1)    # (N, prod(input_shape))
    out = x_rev.dot(w) + b
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        xhat = (x - sample_mean) / (sample_var + eps)
        out = gamma * xhat + beta
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        # 因为在0,1上均匀分布，所以<0.25的比例必然是0.25
        mask = (np.random.rand(x.shape[0], x.shape[1]) < p) / p
        # print(np.random.rand(x.shape[0], x.shape[1]) < p)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    N, C, H, W = x.shape       # C: channel, H:height, W:width
    F, C, HH, WW = w.shape     # F: filter nums
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(H_out):
        for j in range(W_out):
            x_padded_mask= x[:,:,stride*i: stride*i+HH, stride*j: stride*j+WW]  # (:,:, WW, HH)
...
def max_pool_backward_naive(dout, cache):
    ...
    H_out, W_out = x.shape[2:4]    # x: (N,C,H,W)
    print(H_out, W_out)
    for i in range(H_out):
        for j in range(W_out):
            # mask 
            x_padded_mask= x[:,:,stride*i: stride*i+HH, stride*j: stride*j+WW]  # (:,:, HH, WW)
            print(x_padded_mask.shape)

            # print(x_padded_mask[0:2])
            max_mask = np.max(x_padded_mask, axis=(2,3))

            # max_mask = np.max(x_padded_mask, axis=(2,3))
            index_mask = (x_padded_mask == (max_mask)[:,:,None,None])
            dx[:, :, i*stride:i*stride+HH, j*stride:j*stride+WW] += index_mask * (dout[:,:,i,j])[:,:,None,None]
